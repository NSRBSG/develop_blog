...
export default function ThemeSwitch() {
  ...
  return (
    <div className='flex items-center justify-center w-full h-full'>
      <div className='relative inline-block w-16 align-middle select-none transition duration-300 ease-in hover:scale-110'>
        <input
          onChange={() =>
            setTheme(
          ...
          type='checkbox'
          id='toggle'
          className='absolute block w-8 h-8 bg-transparent border-4 rounded-full appearance-none cursor-pointer checked:bg-transparent checked:translate-x-full transition duration-200'
        />
        <label
          htmlFor='toggle'
          className='flex justify-between block h-8 overflow-hidden bg-gray-700 rounded-full cursor-pointer p-1'
        >
          <svg
            style={{ fill: 'white' }}
